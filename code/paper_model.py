...
for val_round in range(validation_times):
    ...
    for t in trange(tfin):
        ...
        if save or show_plot:
            ...
            legend = ['empty', 'type 0 quite', 'type 0 active', 'type 0 arrested',
                      'type 1 quite', 'type 1 active', 'type 1 arrested',
                      'cop']
            patches = [mpatches.Patch(color=colors[i], label=legend[i]) for i in
                       range(len(values))]
